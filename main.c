...
uint8_t i = 0;
uint8_t num_movimiento = 0;
int8_t num_motor = 0;
int16_t grados = 0;
int16_t dato_uart;


char mensaje[] = "Menu I2C";

  int main(void){

eeprom_inicializar();

menu_i2c_inicializar();
...
num_movimiento = esperar_recibir_movimientos();

  for( i = 0; i <= num_movimiento; i++){

    mover_base        (ptr_motores[0]->posicion_uart[i]);
    mover_hombro      (ptr_motores[1]->posicion_uart[i]);
    mover_codo        (ptr_motores[2]->posicion_uart[i]);
    inclinar_pinza    (ptr_motores[3]->posicion_uart[i]);
    rotar_pinza       (ptr_motores[4]->posicion_uart[i]);
    abrir_cerrar_pinza(ptr_motores[5]->posicion_uart[i]);
